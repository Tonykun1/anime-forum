// app/profile/[username]/page.tsx - פשוט וישיר למסד הנתונים שלך
'use client';
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setProfileError(errorData.error || 'משתמש לא נמצא');
        return;
      }
      ...
      // אחרי שהפרופיל נטען, נטען את הפוסטים
      fetchUserPosts();
      
    } catch (err: any) {
      console.error('❌ Error fetching profile:', err);
      setProfileError(`שגיאת חיבור: ${err.message}`);
    } finally {
  ...
  const fetchUserPosts = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setPostsError(`שגיאה ${response.status}: ${errorText}`);
        return;
      }
      ...
      setPostsError(`שגיאת חיבור: ${err.message}`);
    } finally {
      ...
      <div className="min-h-screen bg-gray-900">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-white">טוען פרופיל עבור: {username}</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">
              {profileError || 'משתמש לא נמצא'}
            </h1>
            <div className="space-y-2 mb-4">
              <button
                onClick={fetchUserProfile}
                className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg mx-2 transition-colors"
              >
              ...
              <button
                onClick={() => router.back()}
                className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg mx-2 transition-colors"
              >
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-white">
                פוסטים של {profile.username} ({posts.length})
              </h2>
              <button
              ...
              >
                <RefreshCcw className={`w-4 h-4 ${postsLoading ? 'animate-spin' : ''}`} />
                <span>רענן</span>
              </button>
            </div>
            ...
            {postsLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 mb-4">{postsError}</p>
                <button
                  onClick={fetchUserPosts}
